...
4. Implement `merge_and_split.py` with topic-level splitting.
5. Train the first small LSTM seq2seq baseline.
6. Add ChangeMyView and K-News-Stance adapters.
7. Add AI Hub Korean dialogue adapters after manual data access is resolved.
8. Add optional LLM synthesis and mark all generated examples in metadata.

## Known limitations

- Many Korean sources are not directly debate datasets; they require filtering or synthesis.
- AI Hub datasets may require account-based or manual access. Keep those paths configurable.
